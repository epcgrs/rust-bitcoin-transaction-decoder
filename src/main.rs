use std::io::Read;

fn read_version(mut transaction_bytes: &[u8]) -> u32 {
    
    let mut buffer = [0; 4];

    println!("read_version: slice before read: {:p}", transaction_bytes);

    transaction_bytes.read(&mut buffer).unwrap();

    println!("read_version: slice after read: {:p}", transaction_bytes);
    u32::from_le_bytes(buffer)
}

fn main() {
    let transaction_hex = "";
    let transaction_bytes = hex::decode(transaction_hex).unwrap();
    let mut bytes_slice = transaction_bytes.as_slice();

    println!("Initial slice pointer: {:p}", bytes_slice);

    let version = read_version(bytes_slice);


    println!("After read_version slice pointer: {:p}", bytes_slice);

    // println!("transaction_bytes: {:?}", transaction_bytes);

    let mut input_count = [0; 1];
    bytes_slice.read(&mut input_count).unwrap();
    assert_eq!(input_count[0], 2_u8);

    println!("version {}", version);
}

